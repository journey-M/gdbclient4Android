...
def get_devices(adb_path='adb'):
    ...
    for line in out[1:]:
        if not line.strip():
            continue
        if b'offline' in line:
            continue

        serial, _ = re.split(r'\s+', bytes.decode(line), maxsplit=1)
...
def _get_subprocess_args(args):
    ...
    if (os.name != 'nt' or all(not isinstance(arg, str) for arg in args[0])):
        return args
...
class AndroidDevice(object):
    ...
    def _parse_shell_output(self, out):
        ...
        partition = bytes.decode(search_text).rpartition(self._RETURN_CODE_DELIMITER)
        if partition[1] == '':
